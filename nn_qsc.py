...
def build_neural_network(input_shape, output_shape, reg_strength=0.001):
    inputs = tf.keras.Input(shape=input_shape)
    x = Dropout(0.5)(inputs)
    x = Dense(512, kernel_initializer='he_normal', kernel_regularizer=l2(reg_strength))(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.1)(x)
    x = Dense(256, kernel_initializer='he_normal', kernel_regularizer=l2(reg_strength))(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.1)(x)
    x = Dense(128, kernel_initializer='he_normal', kernel_regularizer=l2(reg_strength))(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.1)(x)
    x = Dense(64, kernel_initializer='he_normal', kernel_regularizer=l2(reg_strength))(x)
    x = LeakyReLU()(x)
    x = BatchNormalization()(x)
    x = Dropout(0.1)(x)
...
model.fit(train_dataset, epochs=params['epochs'], validation_data=validation_dataset, verbose=1, callbacks=[early_stopping, model_checkpoint, reduce_lr])
